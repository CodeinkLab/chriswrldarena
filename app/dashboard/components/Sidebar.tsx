    ...
    <div className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col">
      <div className="flex flex-grow flex-col overflow-y-auto bg-white border-r border-gray-200 pt-5 pb-4">
        <div className="flex flex-shrink-0 items-center px-4">
          <img className="h-8 w-auto" src="/logo.svg" alt="ChrisWrldArena" />
        </div>
        <nav className="mt-8 flex-1 space-y-1 px-2">
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                <div className={`mr-3 flex-shrink-0 ${isActive ? 'text-green-600' : 'text-gray-400 group-hover:text-green-600'}`}>
                  {item.icon}
                </div>
                {item.name}
              </Link>
